...
1. [x] Replace ad-hoc backend POST body parsing with a small, strict parser for the flat JSON objects used by `/api/*`.
...
2. [x] Harden the HTTP request parser and body handling.
...
3. [x] Centralize validation for inbound settings.
...
5. [x] Make SSE line payload allocation exact or dynamically grown.
...
6. [x] Strengthen frontend SSE lifecycle handling.
...
7. [x] Improve frontend API error reporting.
...
8. [x] Throttle frontend localStorage writes.
   Acceptance: visible view state is saved only when it changes and at a bounded rate, not once per waterfall row.

9. [x] Tighten `display_bins` synchronization between canvas resize and backend rows.
...
10. [x] Persist visible range intentionally.
...
11. [x] Fix `clamp_scan_end_to_hardware_limit()` view mutation.
...
12. [x] Make build and run paths configurable.
...
13. [x] Add stronger compiler diagnostics and a lightweight lint/check target.
...
16. [x] Add focused regression tests or test procedures for the hardened paths.
